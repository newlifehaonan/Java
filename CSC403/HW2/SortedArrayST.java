...
public class SortedArrayST<Key extends Comparable<Key>, Value> {
	...
	public int rank(Key key) {
		return logarithmicTimeRank(key);

		// TODO3 : logarithmic time implementation
	}
	
	private int logarithmicTimeRank(Key key) {
		int lo = 0, hi = N-1;
		while (lo <= hi) {
			int m = lo + (hi - lo) / 2;
	...
	public boolean equals(Object x) {
		...
		SortedArrayST<String, String> that = (SortedArrayST<String, String>) x;
		if(this.N!=that.N) return false;
		for(int i=0; i<that.N;i++) {
			if(keysArray()[i].compareTo((Key) that.keysArray()[i])!=0) {
				result = false; break;
			}
			if(!valsArray()[i].equals(that.valsArray()[i])) {
				result = false; break;
			}
		}
		return result;
	...
	public int countRange(Key key1, Key key2) {
		...
		if(cmp<0) {lo = key1;hi = key2;}
		else {lo = key2;hi = key1;}
		int loindex = rank(lo);
		int hiindex = rank(hi);
		int num = hiindex - loindex +2;
		return num; // TODO6
	}
	...
	public static void main(String[] args) {
		...
		//Delete key which is not in ST.
		testDelete("ABDFK","12345", "C","ABDFK","12345");
		//Delete key from empty ST.
		testDelete("","","A","","");
		...
		// Put key-value pair into an empty ST
		testPut("","","A","1","A","1");
		// Put key which has already in ST(Update value of key)//
		testPut("AEIOU","13456","A","2","AEIOU","23456");
		// Test array resizing, because the minimum size of array is 2, So I initiate a ST with size 2 and add one key to see whether the array size doubled
		testPut("AB","12","C","3","ABC","123");
		//    include comments to describe what your case is checking for
	}
	/*
	...
	public static void testPut(String keyInData, String valInData, 
		...
		if (actual.equals(expected)) {
			...
			StdOut.println("Test report");
			StdOut.println("=================================");
			StdOut.println(" Before Put:		After Put:\n");
			StdOut.format("1.      key: %s		%s\n",keyInData, keyOutData);
			StdOut.format("2.    value: %s		%s\n",valInData, valOutData);
			StdOut.format("3.arraysize: %d		%d\n",keyInData.length(),actual.keysArray().length);
			StdOut.println("=================================\n");
		}
		else {
			StdOut.format("testPut: *Error*  Before %s put:%s After: %s, actual: %s, expected: %s\n", keyInData, putKey, keyOutData, actual,expected);
		}
	}
	/*
	...
	public static void testDelete(String keyInData, String valInData, String delKey, 
		...
		SortedArrayST<String, String> expected = from(keyOutData, valOutData);
				...
				if ( actual.equals(expected)) {
					StdOut.format("testDelete: Correct  Before %s delete:%s After: %s\n", keyInData, delKey, keyOutData);// test passes
					StdOut.println("Test report");
					StdOut.println("=================================");
					StdOut.println(" Before Del:		After Del:\n");
					StdOut.format("1.      key: %s		%s\n",keyInData, keyOutData);
					StdOut.format("2.    value: %s		%s\n",valInData, valOutData);
					StdOut.format("3.arraysize: %d		%d\n",keyInData.length(),actual.keysArray().length);
					StdOut.println("=================================\n");
